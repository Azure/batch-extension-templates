...
def wait_for_steady_nodes(batch_service_client: batch.BatchExtensionsClient, pool_id: str, min_required_vms: int, test_timeout: datetime, stop_thread):
    ...
    try:
        ...
        batch_service_client.pool.resize(pool_id, batchmodels.PoolResizeParameter(target_dedicated_nodes = new_node_count))
...
def run_with_jitter_retry(method, *args, retry_count = 0):
    ...
    try:
        ...
        if 'Connection aborted.' in e.message or 'An existing connection was forcibly closed by the remote host' in e.message:
            logger.info("Retrying call as connection forcibly closed by remote host - retryCount: {} of {}".format(retry_count, max_retry_count))
            time.sleep(random.uniform(0.1, 1))  #jitter the next request a bit
            run_with_jitter_retry(method, args, retry_count + 1)
        raise
